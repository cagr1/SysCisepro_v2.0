...
namespace ClassLibraryCisepro3.DivisionGeografica
{
    public class ClassSitiosTrabajo
    {
        ...
        public DataTable SeleccionarItemsByIdSitio(TipoConexion tipoCon, Int32 idSitio, DateTime fechad, DateTime fechah, string grupo)
        {
            ...
            if (idSitio == 0)
            {
                 sql = @"SELECT DK.ID_COMPROBANTE as COMPROBANTE,dk.FECHA, DK.CANTIDAD_INGRESO as CANTIDAD,'I' TIPO, dk.ID_KARDEX, DCIB.OBSERVACION AS SERIE, SI.NOMBRE_SECUENCIAL_ITEM as NOMBRE, CIB.RAZON, ST.NOMBRE_SITIO_TRABAJO AS SITIO
                        ...
                        UNION 
                        SELECT DK.ID_COMPROBANTE as COMPROBANTE, dk.FECHA, DK.CANTIDAD_EGRESO AS CANTIDAD, 'E' TIPO, dk.ID_KARDEX , DCEB.OBSERVACION AS SERIE, SI.NOMBRE_SECUENCIAL_ITEM as NOMBRE, CEB.RAZON, ST.NOMBRE_SITIO_TRABAJO  AS SITIO
                        FROM DETALLE_KARDEX DK 
