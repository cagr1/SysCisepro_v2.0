    ...
    Public Class FormComprobanteEgresoBodega
        ...
        Private Sub tsmAgregar_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles tsmAgregar.Click
            ...
            If Not ValidarKardexRepetidos(CLng(lblIdKardex.Text), lbldetalle.Text) Then
                ...
                If ultimoSaldo < 0 Then
                    KryptonMessageBox.Show("Esta egresando: " & CInt(nudCantidad.Value) & " y el ultimo movimiento tiene de este item tiene:" & _detalleKardex.Rows(0)(19).ToString() & vbCrLf & "Contacte con el administrador", "Mensaje de Validacion", KryptonMessageBoxButtons.OK, KryptonMessageBoxIcon.Information)
                    Return
                End If
        ...
        Private Function ValidarKardexRepetidos(ByVal idKardex As Int64, ByVal detalle As String) As Boolean
            ...
            Try
                ...
                ElseIf _validar = 2 Then

                    For indice = 0 To dgvDetalleComprobanteIngreso.RowCount - 1
                        Dim compareDetalle As String = dgvDetalleComprobanteIngreso.Rows.Item(indice).Cells("DETALLES_INGRESO").Value.ToString()
                        Dim compareSerie As String = ""
                        ...
                        If dgvDetalleComprobanteIngreso.Rows.Item(indice).Cells("ID_KARDEX").Value = idKardex And detalle = compareSerie Then
                            contarRepetidos += 1
                        End If
                    Next
                End If
        ...
        Private Sub tsmGuardar_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles tsmGuardar.Click
            ...
            ElseIf _botonSeleccionadoSitio = 3 Then
                ReingresoComprobante()
                gbxDetalles.BringToFront()
            ...
            ElseIf _botonSeleccionadoSitio = 3 Then
                nombreU = "REINGRESO DE COMPROBANTE " & UserName
            End If
        ...
        Private Sub dgvComprobantesEgreso_SelectionChanged(ByVal sender As System.Object, ByVal e As EventArgs) Handles dgvComprobantesEgreso.SelectionChanged
            ...
            txtIdComprobante.Text = dgvComprobantesEgreso.CurrentRow.Cells.Item(0).Value.ToString().Trim() 'Id_comprobante_ingreso
        ...
        Private Sub ModificarComprobanteEgresoAnular()
            ...
            Try
                For indice = 0 To dgvDetalleComprobate.RowCount - 1
                    Dim idkardex = Convert.ToInt32(dgvDetalleComprobate.Rows(indice).Cells(2).Value)
                ...
                With _objCompIng
                    ...
                    .Estado = 1
                    .Razon = txtRazon.Text.ToUpper.Trim() & " RETORNO"
                    .IdPersonal = If(txtRecibe.Tag Is Nothing, txtRecibe.Text.Split("-")(1).Trim(), CType(txtRecibe.Tag, Integer))
                    .Cliente = txtUbicacion.Text.Trim
                    .IdEmpresa = 1
                    .SitioTrabajo = CInt(txtUbicacion.Tag)
                ...
                Dim idci = _objDetCompIng.BuscarMayorIdDetalleComprobanteIngresoBodega(_tipoCon) + 1
                ...
                For indice = 0 To dgvDetalleComprobate.RowCount - 1
                    ...
                    If Not (Convert.ToInt32(dgvDetalleComprobate.Rows(indice).Cells(2).Value) = 0) Then
                        With _objDetalleKardex
                            ...
                            .CantidadSaldo = CDbl(UltimoMovimiento.Rows(0)(5).ToString()) + CDbl(dgvDetalleComprobate.Rows(indice).Cells(8).Value)
                            .ValorUnitarioSaldo = CDec(CDbl(UltimoMovimiento.Rows(0)(14).ToString()))
                            .ValorTotalSaldo = CDbl(UltimoMovimiento.Rows(0)(5).ToString()) * (CDbl(UltimoMovimiento.Rows(0)(5).ToString()) + CDbl(dgvDetalleComprobate.Rows(indice).Cells(8).Value))
                        ...
                        With _objDetCompIng
                            .IdDetalle = idci
                            .IdKardex = CLng(dgvDetalleComprobate.Rows(indice).Cells(2).Value.ToString.ToUpper)
                            .IdDetalleKardex = _objDetalleKardex.Id
                            .ObservacionCalidadDetalle = dgvDetalleComprobate.Rows.Item(indice).Cells(6).Value.ToString.ToUpper
                            .IdComprobante = _objCompIng.Id
                            .ObservacionDetalleSerial = dgvDetalleComprobate.Rows.Item(indice).Cells(7).Value.ToString.ToUpper & " RETORNO"
                            .Estado = 1
                        End With
                        ...
                        idu += 1
                    Else
                        KryptonMessageBox.Show("El Item " & dgvDetalleComprobate.Rows(indice).Cells(23).Value.ToString() & " tiene valor cero, contacte al Administrador ", "Mensaje de Validacion", KryptonMessageBoxButtons.OK, KryptonMessageBoxIcon.Error)
                        Return
                    End If
        ...
        Public Sub ReingresoComprobante()
            ...
            Try
                ...
                With _objCompIng
                    ...
                    .IdParametroDocumento = cmbDocumento.SelectedValue.ToString()
                    .Estado = 1
                    .Razon = txtRazon.Text.ToUpper.Trim()
                    .IdPersonal = If(txtRecibe.Tag Is Nothing, txtRecibe.Text.Split("-")(1).Trim(), CType(txtRecibe.Tag, Integer))
                    .Cliente = txtUbicacion.Text.Trim
                    .IdEmpresa = 1
                    .SitioTrabajo = CInt(txtUbicacion.Tag)
        ...
        Private Sub btnAgregarIngreso_Click(sender As Object, e As EventArgs) Handles btnAgregarIngreso.Click
            ...
            If Not ValidarKardexRepetidos(CLng(lblIdKardexIngreso.Text), lbldetalleIngreso.Text) Then
                ...
                Dim fila As String() =
                    {
                        lblIdKardexIngreso.Text, 'ID_KAR
                        txtArticuloIngreso.Text, 'NOMBRE_INGRESO 
                        lblIdArticuloIngreso.Text, 'ID_SECUENCIAL_INGRESO
                        cant, 'CANTIDAD_INGRESO
                        val, 'VALOR_INGRESO
                        cant * val, 'TOTAL_INGRESO
                        cbxCalidadIngreso.Text, 'OBSERVACION_INGRESO
                        txtObservacionesIngreso.Text & " - SERIE: " & If(txtSerieIngreso.Text.Trim().Length = 0, "-", txtSerieIngreso.Text.Trim()), 'DETALLES_INGRESO
                        If(_detalleKardexIngreso.Rows(0)(12).ToString().Equals("INGRESO"), CDbl(_detalleKardexIngreso.Rows(0)(14).ToString()), CDbl(_detalleKardexIngreso.Rows(0)(17).ToString())), 'VALOR_UNITARIO_ANTERIOR_INGRESO 
                        If(_detalleKardexIngreso.Rows(0)(12).ToString().Equals("INGRESO"), CDbl(_detalleKardexIngreso.Rows(0)(15).ToString()), CDbl(_detalleKardexIngreso.Rows(0)(18).ToString())), 'VALOR_TOTAL_ANTERIOR_INGRESO
                        CDbl(_detalleKardexIngreso.Rows(0)(19).ToString()) - cant, 'SALDO_INGRESO o SALDO
                        Date.Now(), 'FECHA_INGRESO,
                        _detalleKardexIngreso.Rows(0)(8).ToString(), 'CODIGO_CONCEPTO_INGRESO
                        _detalleKardexIngreso.Rows(0)(10).ToString(), 'CONCEPTO_INVENTARIO
                         cant, 'CANTIDAD_INICIAL_INGRESO
                        (CDbl(_detalleKardexIngreso.Rows(0)(19).ToString()) - cant) * val 'CANTIDAD_TOTAL_INGRESO o VALOR_UNITARIO_SALDO
                    }

                dgvDetalleComprobanteIngreso.Rows.Add(fila)
                dgvDetalleComprobanteIngreso.Rows(dgvDetalleComprobanteIngreso.RowCount - 1).Tag = If(txtSerieIngreso.Text.Trim().Length = 0, "-", txtSerieIngreso.Text.Trim())
                ...
                lblIdKardexIngreso.Text = 0
                _detalleKardexIngreso = Nothing
                nudCantidadIngreso.Enabled = True
                txtSerieIngreso.Clear()
                tsmAgregar.Enabled = False
                tsmEliminar.Enabled = True
        ...
        Private Sub btnBuscarReingreso_Click(sender As Object, e As EventArgs) Handles btnBuscarReingreso.Click
            Dim frm As New FrmBuscarActivoFijo
            frm.TipoCox = TipoCox
            ...
            Try
                If frm.ShowDialog = vbOK Then
                    ...
                    Dim s = frm.dgvActivosFijos.CurrentRow.Cells(6).Value.ToString().Split(".")
                    txtSerieIngreso.Text = s(s.Length - 1)

                    'nudCantidad.Value = 1
                    'nudCantidad.Enabled = False

                    cbxCalidadIngreso.SelectedIndex = 2
                    txtObservacionesIngreso.Text = "OK"
                End If
            Catch
                txtSerieIngreso.Text = "S/N"
                'nudCantidad.Value = 1
                cbxCalidadIngreso.SelectedIndex = 2
            End Try
        End Sub
        ...
        Private Sub btnEliminaringreso_Click(sender As Object, e As EventArgs) Handles btnEliminaringreso.Click
            If dgvDetalleComprobanteIngreso.Rows.Count = 0 Then Return
            dgvDetalleComprobanteIngreso.Rows.RemoveAt(dgvDetalleComprobanteIngreso.CurrentCell.RowIndex())
            tsmEliminar.Enabled = dgvDetalleComprobanteIngreso.RowCount > 0
        End Sub
